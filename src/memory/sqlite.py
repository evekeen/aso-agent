"""SQLite-based memory management for ASO Agent Service."""
...
class AsyncInMemoryStore:
    ...
    async def setup(self):
        # No-op method for compatibility with other store implementations
        pass
...
async def get_sqlite_store():
    ...
    Note: Using InMemoryStore wrapped in async context manager for compatibility.
    SQLite-specific store isn't available in LangGraph, so we use InMemoryStore
    wrapped in an async context manager for compatibility.
    """
